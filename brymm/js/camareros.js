...
function listaCamareros(item) {
    ...
    var contenido = "";
    var contador = 0;
    $(item).find('xml').children('camareroLocal').each(function() {
        if (contador == 0) {
...
//Se obtienen los valores del xml
        ...
        controlTotal = $.trim($(this).find('control_total').text());
...
function llenarFormularioModificarCamarero(idCamarero, nombre, controlTotal) {
    ...
    $("#formModificarCamarero").find('input[name="idCamarero"]').val(idCamarero);
    $("#formModificarCamarero").find('input[name="nombreCamarero"]').val(nombre);

    $("#formModificarCamarero input:checkbox").attr('checked', false);

    if (controlTotal == "1") {
        $("#formModificarCamarero").
                find("input:checkbox[name='controlTotal']").click();
    }
