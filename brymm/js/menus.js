	...
	$(item)
			...
			.each(
					function() {
						...
						idPlatoLocal = $.trim($(this).find('id_plato_local')
								.text());
						idLocal = $.trim($(this).find('id_local').text());
						idTipoPlato = $.trim($(this).find('id_tipo_plato')
								.text());
						...
						// Se crea el enlace para poder borrar los platos del
						// local
						enlaceAnadirMenu = "<a onclick=\"enviarDatosMenu('"
								+ site_url
						...
						if (idTipoPlato != idTipoPlatoAnterior) {
							contenido = contenido + "<h2>" + descripcion
									+ "</h2>";
							contenido = contenido + "<ul>";
						}
						...
						contenido = contenido + nombre + " - " + precio + " - "
...
function listaTipoMenuLocal(item) {
	...
	if ($(item).find('tipoMenuLocal').length > 0) {
		contenido = contenido + "<ul>";
		$(item).find('tipoMenuLocal').each(
				function() {

					// Se obtienen los valores del xml
					nombreMenu = $.trim($(this).find('nombre_menu').text());
					idTipoMenuLocal = $.trim($(this).find('id_tipo_menu_local')
							.text());
					idLocal = $.trim($(this).find('id_local').text());
					...
					enlaceBorrar = "<a onclick=\"doAjax('" + site_url
							+ "/menus/borrarTipoMenuLocal','idTipoMenuLocal="
							+ idTipoMenuLocal
							+ "','listaTipoMenuLocal','post',1)\">B</a>";

					enlaceModificar = "<a onclick=\"doAjax('" + site_url
							+ "/menus/cargarTipoMenuLocal','idTipoMenuLocal="
							+ idTipoMenuLocal
							+ "','cargarTipoMenuLocal','post',1)\">M</a>";

					// Contenido de la lista
					contenido = contenido + "<li>";
					contenido = contenido + nombreMenu + " - " + descripcion
							+ " - " + precioMenu + " - " + enlaceBorrar + " - "
							+ enlaceModificar;
					contenido = contenido + "</li>";
...
function cargarTipoMenuLocal(item) {
	...
	$(item).find('xml').children().each(
			function() {
				...
				$('select[name="idTipoMenuModificar"]').val(idTipoMenu).attr(
						'selected', true);
				$('input[name="idTipoMenuLocalModificar"]')
						.val(idTipoMenuLocal);
				$('input:radio[name="esCarta"][value="' + esCarta + '"]').prop(
						'checked', true);
	...
	$(item)
			...
			.each(
					function() {
						...
						// Se inserta la fecha en el formulario en el campo
						// fechaMenu
						...
						$(this)
								...
								.each(
										function() {
											...
											var enlaceBorrar = "";
											/*
											 * Se obtienen los datos para poder
											 * borrar el plato del menu
											 */
...
function llenarFormularioModificarPlato(nombrePlato, precioPlato, idPlatoLocal,
		idTipoPlato) {
	...
	$("#formModificarPlato").find('input[name="idPlatoLocal"]').val(
			idPlatoLocal);
}
...
$(document).ready(
		function() {
			...
			$("#datepickerFechaPlatoMenu").datepicker("option", "dateFormat",
					"yy-mm-dd");
			$("#datepickerFechaPlatoMenu").val(today);

		});

function mostrarVentanaModificarPlato(nombrePlato, precioPlato, idPlatoLocal,
		idTipoPlato) {
	/*
	 * Ventana modal modificar ingrediente
	 */
	$("#dialogModificarPlato").dialog(
			{
				width : 600,
				modal : true,
				buttons : {
					"Aceptar" : function() {
						...
						enviarFormulario(site_url
								+ '/menus/modificarPlatoLocal',
								'formModificarPlato', 'listaPlatosLocal', 1)
	...
	$('#dialogModificarPlato').dialog('open');
	// Lleno el formulario
	llenarFormularioModificarPlato(nombrePlato, precioPlato, idPlatoLocal,
			idTipoPlato);
	return false;
}

function mostrarVentanaModificarTipoMenu(idTipoMenuLocal) {
	/*
	 * Ventana modal modificar ingrediente
	 */
	$("#dialogModificarTipoMenu").dialog(
			{
				...
				buttons : {
					"Aceptar" : function() {
						// Se envia el formulario que modifica el servicio
						enviarFormulario(site_url
								+ '/menus/modificarTipoMenuLocal',
								'formModificarTipoMenuLocal',
								'listaTipoMenuLocal', 1)
	...
	$('#dialogModificarTipoPlato').dialog('open');
	// Lleno el formulario
	doAjax(site_url + '/menus/cargarTipoMenuLocal', 'idTipoMenuLocal='
			+ idTipoMenuLocal, 'cargarTipoMenuLocal', 'post', 1);
